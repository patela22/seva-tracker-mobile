...
export function SettingsButtonGroup({
  ...
  return (
    <View className="mx-auto mt-2 w-[80vw]">
      ...
      <View className="mt-3 rounded-xl border border-sevatracker-lightgray">
        {buttons.map((button, index) => (
          <View
            key={index + title}
            className={clsx(
              'h-10 w-full justify-center rounded-xl border-sevatracker-lightgray bg-sevatracker-white',
              index !== 0 && 'border-t'
            )}
